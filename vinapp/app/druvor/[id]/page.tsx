...
export default function GrapePage({ params }: Props) {
  const grape = allGrapes.find((g) => g.id === params.id);
  if (!grape) notFound();

  const isWhite = grape.type === 'white';

  // Get top aromas (score >= 3)
      ...
      <section className="mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          {topAromas.map(([id, score]) => {
            ...
            const intensity = score === 5 ? 'Signaturarom' : score === 4 ? 'Mycket vanlig' : 'Vanlig';
            return (
              <div
                key={id}
                title={`${intensity}: ${aroma.description}`}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-colors cursor-default ${
                  score === 5
